  ...
  (__va_start(LIST,__VAstruct,FLAGS),					\
   ...
   ((LIST)->flags & __VA_SUNPROCC_STRUCT_RETURN				\
    ...
    : ((LIST)->flags & __VA_SUNCC_STRUCT_RETURN				\
       ? ((TYPE_SIZE) <= sizeof(vacall_struct_buffer) || vacall_error_struct_too_large(TYPE_SIZE), \
...
#define __va_return(LIST,RETTYPE)  \
  (((LIST)->rtype == (RETTYPE)) || vacall_error_type_mismatch((LIST)->rtype,RETTYPE))
 ...
 */
extern int vacall_error_type_mismatch (enum __VAtype, enum __VAtype);
extern int vacall_error_struct_too_large (unsigned int);
...
extern __va_struct_buffer_t vacall_struct_buffer;
